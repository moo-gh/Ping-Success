...
class PingWorker(QThread):
    ...
    def run(self):
        while not self._stop.is_set():
            ...
            print(f"Ping {self.host}: {'SUCCESS' if success else 'FAILURE'}")  # Debug output
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        basic_line = self.plot.getPlotItem().plot(x_data, y_data, pen=pg.mkPen(color="red", width=10))
        ...
        float_line = self.plot.getPlotItem().plot(x_float, y_float, pen=pg.mkPen(color="green", width=10))
    ...
    def _add_series(self, host: str):
        ...
        dummy_plot = self.plot.getPlotItem().plot([0, 10, 20], [50, 60, 70], pen=pg.mkPen(color="white", width=5))
    ...
    def _replot(self):
        for s in self.series:
            ...
            print(f"History length: {len(s.history)}, data: {list(s.history)[-10:]}")  # Show last 10 values
            ...
            print(f"Plotting {len(x_data)} points, Y range: {min(y_data) if y_data else 0:.1f} to {max(y_data) if y_data else 0:.1f}")
